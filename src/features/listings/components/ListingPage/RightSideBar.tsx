...
export function RightSideBar({ listing }: { listing: Bounty }) {
  ...
  const [isSubmissionNumberLoading, setIsSubmissionNumberLoading] = useState(true);
  ...
  const hasHackathonStarted = Hackathon?.startDate ? dayjs().isAfter(Hackathon.startDate) : true;

  const getSubmissionsCount = async () => {
    setIsSubmissionNumberLoading(true);
    try {
      const submissionCountDetails = await axios.get(`/api/submission/${id}/count/`);
    ...
    <Box w={{ base: 'full', md: 'auto' }}>
      <VStack gap={2} pt={10}>
        <VStack justify="center" gap={0} w={{ base: 'full', md: '22rem' }} bg="#FFFFFF" rounded="xl">
          <VStack justify="space-between" w="full" borderBottom="1px solid #E2E8EF">
            <TableContainer w="full">
              {compensationType !== 'fixed' && (
                <Text mb={1} px={6} pt={3} color="brand.slate.400" fontSize="xs" fontWeight={500}>
              ...
              <Table mt={compensationType === 'fixed' ? -6 : -10} variant="unstyled">
                ...
                <Tbody>
                  <Tr w="full" h={16} borderBottom="1px solid #E2E8EF">
                    <Td colSpan={2}>
                      <Flex align="center" gap={2}>
                        <Image
                          ...
                          rounded="full"
                          src={
                            tokenList.filter((e) => e?.tokenSymbol === token)[0]?.icon ?? '/assets/icons/green-dollar.svg'
                          }
                        />
                        <CompensationAmount
                    ...
                    <Td>
                      {!isProject && (
                        <Text ml={-6} color="brand.slate.400" fontSize="lg" fontWeight={400}>
                          Total Prizes
                        </Text>
                      )}
                    </Td>
                            ...
                            <Tr key={index}>
                              <Td colSpan={2}>
                                <Flex align="center">
                                  ...
                                  <Flex>
                                    <Text color="brand.slate.500" fontSize="lg" fontWeight={500}>
                                      {formatNumberWithSuffix(rewards[prize.key]!)}
                                    </Text>
                                    <Text mt="1px" ml={1} color="brand.slate.400" fontWeight={400}>
                                      {token}
                                    </Text>
                                  </Flex>
                                </Flex>
                              ...
                              <Td>
                                <Text ml={-6} color="brand.slate.400" fontWeight={400}>
                                  {prize.description}
                                </Text>
                              </Td>
                            </Tr>
          ...
          <Flex justify="space-between" w="full" px={5} py={!rewards ? 3 : 0}>
            {hasHackathonStarted ? (
              <>
                <Flex align="start" justify="center" direction="column">
                  <Flex align="center" justify="center" gap={2}>
                    <Image w="1.4rem" mt={-1} alt="suit case" src="/assets/icons/purple-suitcase.svg" />
                    <Text color="#000000" fontSize={{ base: 'lg', md: 'xl' }} fontWeight={500}>
                      {isSubmissionNumberLoading ? '...' : !isProject ? submissionNumber.toLocaleString() : submissionRange}
                    </Text>
                  </Flex>
                  <Text color="#94A3B8">
                    {isProject ? 'Applications' : submissionNumber === 1 ? 'Submission' : 'Submissions'}
                  </Text>
                </Flex>

                <Flex align="start" justify="center" direction="column" py={3}>
                  <Flex align="start" justify="center" gap={1}>
                    <Image w="1.4rem" mt={1} alt="suit case" src="/assets/icons/purple-timer.svg" />
                    <VStack align="start" gap={0}>
                      <Text color="#000000" fontSize={{ base: 'lg', md: 'xl' }} fontWeight={500}>
                        {applicationType === 'fixed' ? (
                          <Countdown date={deadline} renderer={CountDownRenderer} zeroPadDays={1} />
                        ) : (
                          'Rolling'
                        )}
                      </Text>
                      <Text color="#94A3B8">{applicationType === 'fixed' ? 'Remaining' : 'Deadline'}</Text>
                    </VStack>
                  </Flex>
                </Flex>
              </>
            ) : (
              <Flex align="start" justify="centerHere is the improved `rightsidebar.tsx` component with cleaned-up code:
                    ...
                    <Td colSpan={2}>
                      <Flex align="center" gap={2}>
                        <Image
                          ...
                          alt={'green dollar'}
                          rounded={'full'}
                          src={tokenList.filter((e) => e?.tokenSymbol === token)[0]?.icon ?? '/assets/icons/green-dollar.svg'}
                        />
                        <CompensationAmount
                          compensationType={compensationType}
                          rewardAmount={rewardAmount}
                          ...
                          minRewardAsk={minRewardAsk}
                          token={token}
                          textStyle={{ fontWeight: 500, fontSize: { base: 'lg', md: '2xl' }, color: 'brand.slate.700' }}
                        />
                      </Flex>
                    </Td>
                    <Td>
                        ...
                        (prize, index) =>
                          rewards?.[prize.key] && (
                            <Tr key={index}>
                              <Td colSpan={2}>
                                <Flex align="center">
                                  <Flex align={'center'} justify={'center'} w={8} h={8} mr={3} color="brand.slate.500" fontSize={'0.7rem'} bg={'blackAlpha.100'} rounded={'full'}>
                                    {prize.label}
                                  </Flex>
                                  <Flex>
                                    <Text color={'brand.slate.500'} fontSize={'lg'} fontWeight={500}>
          ...
          <Flex justify={'space-between'} w={'full'} px={5} py={!rewards ? 3 : 0}>
            {hasHackathonStarted ? (
              <>
                <Flex align={'start'} justify={'center'} direction={'column'}>
                  <Flex align={'center'} justify={'center'} gap={2}>
                    <Image w={'1.4rem'} mt={-1} alt={'suitcase'} src={'/assets/icons/purple-suitcase.svg'} />
                    <Text color={'#000000'} fontSize={{ base: 'lg', md: 'xl' }} fontWeight={500}>
                      {isSubmissionNumberLoading ? '...' : !isProject ? submissionNumber.toLocaleString() : submissionRange}
                    </Text>
                  </Flex>
                ...
                <Flex align={'start'} justify={'center'} direction={'column'} py={3}>
                  <Flex align={'start'} justify={'center'} gap={1}>
                    <Image w={'1.4rem'} mt={1} alt={'timer'} src={'/assets/icons/purple-timer.svg'} />
                    <VStack align={'start'} gap={0}>
                      <Text color={'#000000'} fontSize={{ base: 'lg', md: 'xl' }} fontWeight={500}>
                        {applicationType === 'fixed' ? (
                          <Countdown date={deadline} renderer={CountDownRenderer} zeroPadDays={1} />
              ...
              <Flex align={'start'} justify={'center'} direction={'column'} py={3}>
                <Flex align={'start'} justify={'center'} gap={1}>
                  <Image w={'1.4rem'} mt={1} altHere's the corrected `RightSideBar.tsx` file with the necessary Prettier adjustments:
